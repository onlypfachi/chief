...
// Eval result collector — accumulates test results, writes to ~/.chief-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.chief', 'test-transcripts');
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with chief installed globally, the fallback path
    // ~/.claude/skills/chief/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.chief/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a chief issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toContain('Hey chief team');
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a chief skill. The session preamble detected _SESSIONS=4 (the user has 4 chief windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.chief/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.chief', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.chief/projects/', async () => {
...
describeIfSelected('chief-upgrade E2E', ['chief-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'chief-remote-'));
    ...
    const mockChief = path.join(upgradeDir, '.claude', 'skills', 'chief');
    fs.mkdirSync(mockChief, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockChief);
    run('git', ['config', 'user.email', 'test@test.com'], mockChief);
    run('git', ['config', 'user.name', 'Test'], mockChief);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockChief);

    // Write old version files
    fs.writeFileSync(path.join(mockChief, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockChief, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockChief, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockChief);
    run('git', ['commit', '-m', 'initial'], mockChief);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockChief);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockChief, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockChief, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockChief);
    run('git', ['commit', '-m', 'release 0.6.0'], mockChief);
    run('git', ['push', 'origin', 'HEAD:main'], mockChief);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockChief);

    // Copy chief-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'chief-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'chief-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'chief-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('chief-upgrade-happy-path', async () => {
    const mockChief = path.join(upgradeDir, '.claude', 'skills', 'chief');
    const result = await runSkillTest({
      prompt: `Read chief-upgrade/SKILL.md for the upgrade workflow.

You are running /chief-upgrade standalone. The gstack installation is at ./.claude/skills/chief (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/chief — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'chief-upgrade-happy-path',
      runId,
    });

    logCost('/chief-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockChief, 'VERSION'), 'utf-8').trim();
    const output = result.output || '';
    ...
    recordE2E('/chief-upgrade happy path', 'chief-upgrade E2E', result, {
