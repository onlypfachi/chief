...
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.chief/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
| Mode | full | `--regression .chief/qa-reports/baseline.json` |
| Output dir | `.chief/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .chief/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.chief/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   source <(~/.claude/skills/chief/bin/chief-slug 2>/dev/null)
   ls -t ~/.chief/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .chief/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.chief/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
source <(~/.claude/skills/chief/bin/chief-slug 2>/dev/null) && mkdir -p ~/.chief/projects/$SLUG
```
Write to `~/.chief/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
