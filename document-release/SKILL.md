...
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.chief/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.chief/*" -not -path "./.context/*" | sort
```
...
```bash
gh pr view --json body -q .body > /tmp/chief-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/chief-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/chief-pr-body-$$.md
```
