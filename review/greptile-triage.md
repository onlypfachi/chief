...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/chief/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.chief/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/chief/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.chief/projects/$REMOTE_SLUG"
mkdir -p ~/.chief
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.chief/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.chief/greptile-history.md` (global aggregate)
