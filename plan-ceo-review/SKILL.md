...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.chief/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
**Design doc check:**
```bash
SLUG=$(~/.claude/skills/chief/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.chief/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.chief/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
source <(~/.claude/skills/chief/bin/chief-slug 2>/dev/null) && mkdir -p ~/.chief/projects/$SLUG/ceo-plans
```
...
```
NOTE: These represent human-team implementation hours. With CC + chief,
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+chief: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
```bash
~/.claude/skills/chief/bin/chief-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
...
- **B)** Keep in `~/.chief/projects/` only (local, personal reference)
- **C)** Skip
