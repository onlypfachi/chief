...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/chief-cli-');
    const execPath = path.join(root, '.claude/skills/chief/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/chief/browse/src/server.ts');
