/**
 * Tests for bin/chief-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * CHIEF_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'chief-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'chief-config-test-'));
});
...
describe('chief-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { CHIEF_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
