#!/usr/bin/env bun
/**
 * analytics — CLI for viewing chief skill usage statistics.
 *
 * Reads ~/.chief/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.chief', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('chief skill usage analytics');
